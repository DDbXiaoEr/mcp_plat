...
> **国际化**：`message` 字段支持中英双语。客户端可通过请求头 `Accept-Language` 声明语言偏好：
> - 携带 `en` / `en-*`（如 `Accept-Language: en-US`）时，`message` 返回英文（近似翻译，未收录的拼接片段保留原文）；
> - 缺省或中文（`zh` / `zh-*`）时返回中文原文。
> 业务 `data` 为用户数据不翻译；`data` 中少数由后端生成的兜底标签（如 LDAP 测试结果 `message`、概况图表的 `未知服务器`/`其他`/`未分组`）会随语言本地化。
...
无鉴权，进程存活即返回 200，供 Docker / K8s 存活探针使用。
...
无鉴权，进程存活且数据库可达才返回 200，否则返回 503，供 Docker / K8s 就绪探针使用。
